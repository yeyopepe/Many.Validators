...
namespace Many.Validators.Tests
{
	...
	{
		readonly int?[] _inputErrors = new int?[] { -1, 0 };
		...
		public void IsValid_ReturnsFirstError(bool includeErrors, bool expectedResult)
		{
			...
			//Test
			var result = Positive<int?>.IsValid(values: input,
		...
		public void Validate_MultipleValues_ThrowsException()
		{
			...
			//Test
			Assert.Throws<ArgumentOutOfRangeException>(() => Positive<int?>.Validate(values: input));
		}
		[Test]
		public void Validate_MultipleValues_Returns()
		{
			...
			//Test
			Assert.DoesNotThrow(() => Positive<int?>.Validate(values: input));
		}
	}
}
